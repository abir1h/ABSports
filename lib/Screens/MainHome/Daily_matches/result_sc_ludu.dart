...
class _result_scState extends State<result_sc> {
  var type, mobile, g_id, try_value, win, p_kill, entryfee, time;
  var player_name, kills, winnning;
  bool haswinner = false;
  bool data_get = false;
  var datalength;
  Future<List<medicine2>> fetchMEdicine() async {
    ...
    var response = await http.get(Uri.parse(AppUrl.result_details + widget.id),
    ...
    if (response.statusCode == 200) {
      ...
      print(jsonResponse);
      return jsonResponse.map((job) => new medicine2.fromJson(job)).toList();
    } else {
      setState(() {
        has_medicine2 = false;
      });
              ...
              data_get == true
                  ...
                  child: Column(
                    children: [
                      ...
                      try_value > 0
                          ? has_medicine2 == true
                          ? FutureBuilder<List<medicine2>>(
                        future: result_d,
                        builder: (ctx, snapshot) {
                          if (snapshot.hasData) {
                            print(snapshot.data);
                            List<medicine2> data = snapshot.data;
                            print(data);
                            // print(data);
                            return SingleChildScrollView(
                              scrollDirection: Axis.vertical,
